...
def _run_script(
    ...
    *,
    ...
    if not script.exists():
        raise McpError(-32603, f"script not found: {script.relative_to(ROOT)}")
...
def _load_json(path: Path) -> Any:
    if not path.exists():
        raise McpError(-32603, f"file not found: {path.relative_to(ROOT)}")
    try:
...
def tool_validate_repo(_params: dict[str, Any]) -> dict[str, Any]:
    result = _run_script(SCRIPTS_DIR / "validate_repo.py", [], timeout=180)
    ok = result.returncode == 0
    return _tool_text(
        "Repository validation passed." if ok else "Repository validation failed.",
        {"ok": ok, "stdout": result.stdout, "stderr": result.stderr},
    )
...
def tool_arc_builder_doctor(params: dict[str, Any]) -> dict[str, Any]:
    ...
    result = _run_script(SCRIPTS_DIR / "arc_builder_doctor.py", args, timeout=240)
    try:
        report = json.loads(result.stdout) if result.stdout.strip() else {}
    except json.JSONDecodeError:
        report = {"raw": result.stdout, "error": "doctor output was not valid JSON"}
    ok = result.returncode == 0 and report.get("status") in ("pass", "warn")
    return _tool_text(
        f"Doctor report status: {report.get('status', 'unknown')}",
        {"ok": ok, "report": report, "stderr": result.stderr},
    )
