...
def run_doctor_report() -> tuple[dict[str, Any], str]:
    ...
    """
    # Ensure the sibling script directory is importable when run directly.
    scripts_dir = str(ROOT / "scripts")
    if scripts_dir not in sys.path:
        sys.path.insert(0, scripts_dir)

    import arc_builder_doctor as doctor

    options = doctor.Options()
    report = doctor.build_report(options)
    markdown = doctor.render_markdown(report)
...
def main(argv: list[str] | None = None) -> int:
    ...
    facts_path = ROOT / "config" / "arc_testnet.facts.json"
    try:
